...
scatter(1:10, rand(10), rand(10))
scatter(rand(12))
plot!(rand(12))
# poly(FRect3D((1,2,3),(0.3,0.5,0.8)))
# text() do io
#     println(io,"foooooooooooooo")
# end
text("ooooooo")
...
# text(y, position = (500, 800), textsize = 50, align = (:center, :top))
...
grid_size =
    @lift((widths($area)[1] / $game.height, widths($area)[2] / $game.width))

snake_boxes = @lift([
    FRect2D((p.I .- (1, 1)) .* $grid_size, $grid_size) for p in $game.snake
])
poly!(scene, snake_boxes, color = :blue, strokewidth = 5, strokecolor = :black)

snake_head_box =
    @lift(FRect2D(($game.snake[1].I .- (1, 1)) .* $grid_size, $grid_size))
...
scatter!(
    ...
    marker = '◉',
    color = :blue,
    markersize = @lift(minimum($grid_size)),
)
...
text!(
    ...
    score_text,
    color = :gray,
    position = @lift((widths($area)[1] / 2, widths($area)[2])),
    textsize = 50,
    align = (:center, :top),
)
